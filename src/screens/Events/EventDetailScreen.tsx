...
import {
    ...
    SafeAreaView, StatusBar, Alert, ActivityIndicator, Linking, Image
} from 'react-native';
...
const EventDetailScreen = ({ route, navigation, activeTheme: propsTheme }: any) => {
    ...
    useEffect(() => {
        ...
        const unsubFav = firestore().collection('Favorites')
            .where('userId', '==', currentUser.uid).where('jobId', '==', item.id)
            .onSnapshot(snap => setIsFavorite(!snap?.empty));

        const unsubJoin = firestore().collection('Applications')
            .where('userId', '==', currentUser.uid).where('jobId', '==', item.id)
            .onSnapshot(snap => {
    ...
    const toggleFavorite = async () => {
        try {
            ...
            const query = await favRef.where('userId', '==', currentUser?.uid).where('jobId', '==', item.id).get();

            if (query.empty) {
                await favRef.add({
                    userId: currentUser?.uid, jobId: item.id, jobData: item,
                    type: 'event', addedAt: firestore.FieldValue.serverTimestamp()
                });
            } else {
    ...
    const handleJoinEvent = async () => {
        if (isJoined) return;
        Alert.alert("Etkinliğe Katıl", "Listene eklemek istiyor musun?", [
            { text: "Vazgeç", style: "cancel" },
            {
                text: "Evet", onPress: async () => {
                    try {
                        await firestore().collection('Applications').add({
                            userId: currentUser?.uid, jobId: item.id, jobTitle: item.title,
                            companyName: item.companyName || 'Kurumsal', appliedAt: firestore.FieldValue.serverTimestamp(),
                            status: 'Katıldı', type: 'event', jobData: item
                        });
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: activeTheme.background }]}>
            ...
            <View style={styles.topBar}>
                ...
                <TouchableOpacity onPress={toggleFavorite}>
                    <Feather name="heart" size={26} color={isFavorite ? "#EF4444" : activeTheme.text} />
                </TouchableOpacity>
            </View>
